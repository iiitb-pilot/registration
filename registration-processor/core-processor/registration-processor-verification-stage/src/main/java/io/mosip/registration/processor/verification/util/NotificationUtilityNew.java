...
import io.mosip.registration.processor.verification.dto.MessageSenderDTO;
...
@Component
public class NotificationUtilityNew {

	/** The reg proc logger. */
	private static Logger regProcLogger = RegProcessorLogger.getLogger(NotificationUtilityNew.class);
	...
	private static final String NOTIFICATION_TEMPLATE_CODE="regproc.packet.verification.notification.template.code.";
	...
	public void sendNotification(IndividualDemographicDedupe demographicData,
		...
		String Name = null;		
        JsonValue[] json = demographicData.getName().get(0);
        for (JsonValue json1 : json) {           
        	Name=json1.getValue() ;       
        }		
		...
		if (allNotificationTypes != null) {
			for (String notificationType : allNotificationTypes) {
				if (notificationType.equalsIgnoreCase("EMAIL")
						&& (demographicData.getEmail() != null
						&& !demographicData.getEmail().isEmpty())) {
					sendEmailNotification(demographicData, messageSenderDTO, attributes, description,preferredLanguage);
				} else if (notificationType.equalsIgnoreCase("SMS") && (demographicData.getPhone() != null
						&& !demographicData.getPhone().isEmpty())) {
					sendSMSNotification(demographicData, messageSenderDTO, attributes, description,preferredLanguage);
				}
			}
		}
		}
	...
	PacketManagerException, JsonProcessingException, IOException, JSONException {
		if(userPreferredLanguageAttribute!=null && !userPreferredLanguageAttribute.isBlank()) {
			...
			String preferredLang=packetManagerService.getField(registrationStatusDto.getRegistrationId(), userPreferredLanguageAttribute,
				registrationStatusDto.getRegistrationType(), ProviderStageName.VERIFICATION);
	...
	private void sendSMSNotification(IndividualDemographicDedupe demographicData,
			MessageSenderDTO messageSenderDTO, Map<String, Object> attributes, LogDescription description,String preferedLanguage) {
		try {
			SmsResponseDto smsResponse = sendSMS(demographicData,
	...
	private SmsResponseDto sendSMS(IndividualDemographicDedupe demographicData, String templateTypeCode,
		...
		try {
			...
			smsDto.setNumber(demographicData.getPhone());
	...
	private void sendEmailNotification(IndividualDemographicDedupe demographicData,
			MessageSenderDTO messageSenderDTO, Map<String, Object> attributes, LogDescription description,String preferedLanguage) {
		try {
			String subjectTemplateCode = messageSenderDTO.getSubjectTemplateCode();
			
			ResponseDto emailResponse = sendEmail(demographicData,
	...
	private ResponseDto sendEmail(IndividualDemographicDedupe demographicData, String templateTypeCode,
		...
		try {
			...
			String mailTo = demographicData.getEmail();
